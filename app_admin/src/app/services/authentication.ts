...
export class Authentication {
  ...
  public saveToken(token: string): void {
    ...
    console.log("token stored and saved")
  }
  ...
  try {
    ...
    let payloadmodified = payload.exp > Date.now() / 1000;
    console.log(payloadmodified);
  ...
  public login(user: User, passwd: string) {
    this.tripDataService.login(user,passwd)
      .subscribe({
        next: (value: any) => {
          if(value) {
            console.log('Raw API response:', value);
            this.authResp.token = value;
            this.saveToken(this.authResp.token);
            console.log("Token: " + this.authResp.token);
          }},
